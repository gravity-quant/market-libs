...
from iol_client.exceptions import IOLClientError, IOLDecodeError
...
class _Model:
    """Base carrying the ``from_api`` shape Phase 30 will generate."""
...
class _Cotizacion(_Model):
    """A plausible Phase 30 shape: the fields an IOL quote payload carries."""
...
def test_policy_constant_matches_the_semantics_matrix() -> None:
    """``29-SEMANTICS-MATRIX.md`` Section 2, iol row — the higyrus constant.
...
def test_logger_name_is_this_package() -> None:
    ...
    assert _decode._LOGGER_NAME == "iol_client"


def test_context_var_names_are_package_prefixed() -> None:
    """Two more normalized lines: a shared name would alias across packages."""
    assert _decode.STRICT_DECODE.name == "iol_client_strict_decode"
    assert _decode.DECODE_SCOPE.name == "iol_client_decode_scope"


def test_decode_scope_declaration_is_reflowed_by_the_formatter() -> None:
    """Plan 09 hand-off: substituting the ContextVar name can change LINE COUNT.

    ``iol_client`` is short enough that the whole ``DECODE_SCOPE`` assignment
    fits inside the 100-column limit, so ``ruff format`` collapses the
    three-line call that higyrus / market-data / matriz all carry into one. The
    intactness normalizer must therefore compare the two declarations
    semantically (or re-format both sides) rather than line-for-line — a naive
    package-name substitution followed by a byte compare would report a false
    divergence here and only here.
    """
    source = pathlib.Path(inspect.getfile(_decode)).read_text(encoding="utf-8")
    declaration = next(
        line for line in source.splitlines() if line.startswith("DECODE_SCOPE: ContextVar")
    )

    assert declaration.endswith('ContextVar("iol_client_decode_scope", default=None)')
...
def test_decode_module_never_imports_models() -> None:
    ...
    package_imports = {name for name in imported if name.startswith("iol_client")}
...
def test_record_is_flat_all_str_and_carries_no_wire_value(
    ...
    for record in records:
        ...
        assert record.package == "iol_client"  # type: ignore[attr-defined]
...
def test_strict_mode_raises_with_the_exact_field_path_and_no_wire_value() -> None:
    ...
    try:
        with pytest.raises(IOLDecodeError) as excinfo:
            walk_model(
                _Scalars,
    ...
    assert isinstance(excinfo.value, IOLClientError)
...
def test_strict_mode_raises_on_missing() -> None:
    ...
    token = _decode.STRICT_DECODE.set(True)
    try:
        with pytest.raises(IOLDecodeError, match=r"\.s"):
...
def test_strict_mode_raises_on_non_dict() -> None:
    ...
    token = _decode.STRICT_DECODE.set(True)
    try:
        with pytest.raises(IOLDecodeError, match="_Scalars"):
...
def test_decode_error_is_not_an_api_error() -> None:
    """The HTTP response succeeded; it is the payload shape that failed."""
    error = IOLDecodeError(".precio", "float", "str", "_Cotizacion")

    assert isinstance(error, IOLClientError)
    assert not isinstance(error, iol_client.IOLAPIError)
...
def test_from_api_shape_reports_a_missing_float_and_still_substitutes(
    ...
    """The exact pairing Phase 30 inherits: ``0.0`` returned AND reported."""
    payload = _full_payload(_Cotizacion)
...
def test_from_api_shape_nested_path_is_dotted_from_the_decode_root(
    ...
    """``.puntas[].dias`` — the aggregation contract's worked example, iol-side."""
    payload = _full_payload(_Cotizacion)
    payload["puntas"] = [{"simbolo": "GGAL"}, {"simbolo": "YPFD"}]

    caplog.clear()
    with caplog.at_level(logging.DEBUG, logger="iol_client"):
        obj = _Cotizacion.from_api(payload)

    assert len(obj.puntas) == 2
    assert _tuples(_divergences(caplog)) == [(".puntas[].dias", "missing")]


def test_from_api_shape_is_fatal_under_strict_mode() -> None:
    """Both modes reachable from the shape Phase 30 will generate."""
    payload = _full_payload(_Cotizacion)
    del payload["ultimoPrecio"]

    token = _decode.STRICT_DECODE.set(True)
    try:
        with pytest.raises(IOLDecodeError, match=r"\.ultimoPrecio"):
...
def _restored_default_clients() -> Iterator[None]:
    ...
    try:
        ...
        iol_client.configure(strict_decode=False)
        aio.configure(strict_decode=False)


def _spec() -> RequestSpec:
    return RequestSpec(method="GET", path="/api/v2/estadocuenta", endpoint_name="get_estadocuenta")
...
def test_strict_mode_from_configure() -> None:
    ...
    with _restored_default_clients():
        iol_client.configure(strict_decode=True)
        ...
        # call must NOT silently reset a previous opt-in.
        iol_client.configure(base_url="https://api.test")
...
def test_strict_mode_bound_by_module_shim(httpx_mock: HTTPXMock) -> None:
    ...
    with _restored_decode_context(), _restored_default_clients():
        ...
        client_mod._request("GET", "/api/v2/estadocuenta")
        assert _decode.STRICT_DECODE.get() is True
        assert isinstance(_decode.DECODE_SCOPE.get(), DecodeScope)
