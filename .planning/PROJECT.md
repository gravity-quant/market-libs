...
## Current Milestone: v1.3 Codegen Single-Source (libcst)
...
<!-- v1.3 scope — Codegen Single-Source (libcst). REFAC-06 spike-gated por D-NOGO-01. -->

- [ ] **CODEGEN-01**: `libcst >=1.8.0,<2` evaluado como herramienta de codegen AST-level contra un gate `D-RIGOR-02` de 10 items sobre el canary ámbito (forma v1.2-head, **sin** migración de source) + inheritance de la matriz construct audit (109 rows, 0 unresolved) + deny-list intactness; decisión GO/NO-GO firmada por operator. Los 8 items heredados de SPIKE-005 (B8 identity, `@generated` marker, ruff format/check, mypy strict, lint-imports, future-import compat) + 2 nuevos libcst-específicos (CSTTransformer purity, deny-list bajo MetadataWrapper).
- [ ] **REFAC-06** *(CONDITIONAL on CODEGEN-01 GO)*: los transport shells `client.py`/`aio.py` de los 4 paquetes verificables (ámbito, iol, higyrus, matriz) se single-sourcean vía libcst codegen — `@generated` marker (PEP 263/236), CI `lint-codegen` verify-clean job, B8 identity (`_raise_for_response is _core.raise_for_response`) preservada, `ruff`/`mypy --strict`/`import-linter` clean, suites mockeadas verdes vs generado. Si SPIKE-006 items 1/4/6 FAIL de nuevo → REFAC-06 se archiva permanentemente (shells duplicados = feature estructural aceptado).
