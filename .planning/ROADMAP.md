...
**Success Criteria** (what must be TRUE):
  ...
  5. If decision = NO-GO: REQUIREMENTS.md and ROADMAP.md updated to defer REFAC-06 to v1.3; Phase 16 dropped from the schedule; Phase 17 unblocked to run immediately after Phase 14 + 15.**Plans:** 4/3 plans complete

**Wave 1**

- [x] 12-01-PLAN.md — Wave 0 + Wave 1: bootstrap SPIKE-005 directory + sub-experiment skeletons + MANIFEST registration + 12-VALIDATION.md fill + Ámbito round-trip experiment (001a) end-to-end (unasync round-trip + ruff format + diff + B8 identity assert)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 12-02-PLAN.md — Wave 2 + Wave 3 + Wave 4: @generated marker × from __future__ compatibility (001b) + matriz construct audit ast.walk + operator triage zero-TBD merge gate (001c) + matriz deny-list intactness sha256 before/after (001d)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 12-03-PLAN.md — Wave 5 + Wave 6: D-RIGOR-01 8-item evidence checklist execution + 1-day timebox check + operator GO|NO-GO signoff + GO branch wrap-up Skill + auto-load OR NO-GO branch REQUIREMENTS/ROADMAP/pending-todo updates + skill NO-GO flavor + 12-SUMMARY.md
...
- [x] 13-01-PLAN.md — Cross-cutting tests-first + RetryTransport `max_attempts` extension wiring × 4 packages × 2 surfaces (Wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 13-02-PLAN.md — `with_options` ámbito-financiero-client (canary, no auth) (Wave 2)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 13-03-PLAN.md — `with_options` higyrus-client + RedactingFilter integrity (Wave 3)
...
- [x] 14-01-PLAN.md — Cross-cutting tests-first + platformdirs dep on iol-client (RED in HEAD per D-P1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 14-02-PLAN.md — `_token_cache.py` + `_state.py` field + sync `Client` integration (3 CRITICAL gates + 4 sync regression GREEN)
...
- [x] 15-01-PLAN.md — ámbito driver migration + first AST-guard test + LOC-attestation anchor (Wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 15-02-PLAN.md — iol driver migration + AST guard (forced-refresh write-site, Wave 2)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 15-03-PLAN.md — higyrus driver migration + AST guard (~19 probes, Wave 3)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 15-04-PLAN.md — matriz driver migration + AST guard + final stability gate + ≥907-test attestation (Wave 4)
...
**Status:** DROPPED. Phase 12 returned NO-GO on 2026-06-14 (signed by sebadlf, strict
D-RIGOR-01 reading — 3 of 8 evidence items FAIL: items 1 byte-identical, 4 ruff check,
6 ámbito pytest; all trace to source-shape asymmetry; 0 Recipe-2 class-3 unfixable
hunks). REFAC-06 deferred to v1.3 with a dedicated libcst spike per locked decision
D-NOGO-01. See `.planning/spikes/SPIKE-005-codegen-tool-choice/NO-GO.md` for root cause
analysis + `.planning/todos/pending/spike-codegen-libcst-v1.3.md` for v1.3 scope.

**Original goal (informative — for v1.3 carry-forward):** ONE source of truth for
sync/async transport shells × 4 packages — operators editing endpoint logic touch one
file (`aio.py`), pre-commit regenerates `client.py`, CI verifies idempotency.

**Depends on (informative):** v1.3 libcst spike GO decision; v1.2 Phase 13 (with_options surface stable); v1.2 Phase 15 (driver migration validates public method surface before codegen masks API gaps); v1.1 Phase 7 `_core.py` extraction (codegen targets transport shells ONLY).
**Requirements**: REFAC-06 (deferred to v1.3)
**Success Criteria** (what must be TRUE):
...
**Goal**: An operator running `main_*.py --live × 4` post-migration confirms no new findings outside the in-cycle classified set vs baseline `verification-cycle-2026-Q2` + v1.1 LIVE-01 head `71bf201`; the milestone v1.2 ships with audit `passed`. **Unblocked early per SPIKE-005 NO-GO 2026-06-14** — Phase 16 dropped from schedule; Phase 17 runs immediately after Phase 14 + 15 with no codegen gate.
**Depends on**: All preceding v1.2 phases complete (Phase 13 ERG-01 + Phase 14 SEC-01 + Phase 15 REFAC-05). Phase 16 dropped per Phase 12 NO-GO; no codegen prerequisite.
...
- [x] 17-01-PLAN.md — Deterministic gate-readiness: fix iol F-02 prose-only Regression link so `verify_cycle_closure × 4` PASS; attest D-06 static title-stability + sync/async isolation + credential-leak + public-surface gates GREEN + pytest ≥989 (autonomous)

**Wave 2** *(blocked on Wave 1; operator-driven)*

- [x] 17-02-PLAN.md — Operator live gate × 4 + `17-VALIDATION.md` pre-fill/approval handshake + per-package RAN/SKIPPED-EXPECTED dispositions (D-02) + iol F-01 re-confirm OPEN (D-05) + conditional D-07 new-finding disposition + regression (`autonomous: false`)
