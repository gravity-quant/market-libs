...
4. A read-only ``@property`` alias declared next to the wire fields of a
   ...
   Phase 38 depends on when it turns ``Cotizacion.puntas`` into a
   ``list[Punta]`` and ``Titulo.puntas`` into a ``Punta`` Null Object
   (ROADMAP Phase 35 criterio 5, D-16).

iol takes **form A** of D-07: this paquete's ``from_api`` carries no mapping
pass, so neither does its ``empty()``. The market-data twin (form B) does, and
that delta is a declared per-paquete policy axis, not a divergence to harmonize.
...
from iol_client import _decode, models
from iol_client._decode import POLICY, DecodeScope, walk_model
from iol_client.models import SafeModel
...
class _AliasShaped(SafeModel):
    """The exact shape Phase 38 introduces: wire fields + a read-only alias."""
...
def test_the_model_roster_is_not_vacuous() -> None:
    ...
    ``>= 4`` rather than ``== 4``: 4 is the count measured for iol in
    ``35-RESEARCH.md`` F-1 (``Punta``, ``Cotizacion``, ``Instrumento``,
    ``Titulo``), and Phase 38 may legitimately add classes. Every class is also
    asserted individually, so the guard only has to catch the catastrophic case
    where introspection returns nothing at all.
    """
    assert len(_safemodel_classes()) >= 4
