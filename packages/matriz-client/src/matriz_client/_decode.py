"""The canonical per-field decode walker for matriz-client (Phase 29, DEC-01).
...
Who imports it: :mod:`matriz_client.models` only. ``_SafeModel.from_api``
delegates to :func:`walk_model` and the ``_convert`` back-compat shim delegates
...
STRICT_DECODE: ContextVar[bool] = ContextVar("matriz_client_strict_decode", default=False)
DECODE_SCOPE: ContextVar[DecodeScope | None] = ContextVar(
    "matriz_client_decode_scope", default=None
)
