...
Phase 20 market-data. CORE-MD-01 + D-10. Mechanical duplicate of the iol
transport with a package-specific logger name:
``_LOGGER_NAME = "market_data_client"`` so structured log records land on the
market-data package logger (which gets its own ``RedactingFilter`` attached in
a later wave covering the Auth0 bearer token).

Invariants (identical to iol):
...
- Retryable surfaces (D-07): HTTP status ``408/409/429/5xx`` + transport-level
  ...
  (``MarketDataAPIError``/``MarketDataAuthError``/``MarketDataRateLimitError``)
...
- 401 re-auth (D-02): NOT this layer. ``MarketDataAuthError`` NEVER in ``retry_on=`` —
...
flow through ``logging.getLogger("market_data_client")`` which has the
...
_LOGGER_NAME = "market_data_client"
...
class _RetryableStatus(Exception):
    ...
    NOT a subclass of ``MarketDataAPIError`` — keeps D-07 invariant (domain
