...
- [x] 09-04-PLAN.md — Green gate consolidation: full pytest matrix + ruff + mypy strict + lint-imports + cross-leak sentinel + snapshot zero-diff + operator checkpoint (BUG-01..04)
...
- [x] 10-03-PLAN.md — `_state.py` +1 field `token_store` + sync/async/ws_client migration to `state.token_store.{get_sync,get_async}()` + cross-thread regression test (+5 tests); ORP-01 `account_id` preserved (REFAC-04)
...
- [x] 11-02-PLAN.md — Code Review mega-plan (CR-01/02/04/06/07/08) + spike artifacts ruff housekeeping (Phase 8 D-21 per-CR atomic commits, risk-first order D-CR-03)
