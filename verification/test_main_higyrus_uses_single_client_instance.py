"""REFAC-05 -- ``main_higyrus.py`` MUST construct at most one sync ``Client`` and
...
``higyrus_client.client._get_default()`` / ``aio._get_default()`` (plus the one
method-call-on-default ``await aio._get_default()._ensure_http_client()``). The
driver migration (Phase 15) replaces those with exactly one ``Client()`` built in
...
def test_main_higyrus_uses_single_client_instance() -> None:
