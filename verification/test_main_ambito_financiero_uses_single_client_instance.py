"""REFAC-05 -- ``main_ambito_financiero.py`` MUST construct at most one sync
...
``ambito.client._get_default()`` / ``aio._get_default()``. The driver migration
(Phase 15) replaces those with exactly one ``Client()`` built in ``main()`` and one
``AsyncClient()`` built in ``_async_main()``, both threaded as parameters into every
``probe_*``. This AST gate is the merge-blocking invariant: it counts constructor
``Call`` sites for the two client classes across the whole driver and asserts
``1 <= count <= 2`` -- the lower bound makes the gate non-vacuous (a driver that
constructs ZERO classes, i.e. the un-migrated state, FAILS RED rather than passing
trivially).

The walker matches BOTH constructor spellings per D-05: bare ``Client()`` /
``AsyncClient()`` (``ast.Name``) and module-qualified ``ambito.Client()`` /
...
_DRIVER = "main_ambito_financiero.py"
...
def test_main_ambito_financiero_uses_single_client_instance() -> None:
