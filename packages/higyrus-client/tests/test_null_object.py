"""Null Object contract for ``higyrus_client.models`` — NOBJ-01, Phase 35.

This suite pins four things, and it is deliberately the FIRST place in the
monorepo where they are pinned (higyrus is the tracer paquete of Phase 35):

1. Every shipped :class:`~higyrus_client.models.SafeModel` subclass is **falsy**
...
4. A read-only ``@property`` alias declared next to the wire fields of a
   ...
   phases 36-38 depend on when they add ``last`` / ``bids`` aliases
   (ROADMAP Phase 35 criterio 5, D-16). Retired here, before those phases can
...
from higyrus_client._decode import POLICY, DecodeScope, walk_model
...
def _perturb(empty: Any) -> Any:
    ...
    The trailing nested-model branch is the higyrus-local counterpart of that
    problem: ``models.Administrador`` declares three fields and all three are
    nested models with zero scalars, so its empty instance offers no ``None``,
    no ``str`` and no container to perturb — only nested empty instances, which
...
def test_the_model_roster_is_not_vacuous() -> None:
    ...
    ``>= 15`` rather than ``== 15``: 15 is the count measured for higyrus in
    ``35-RESEARCH.md`` F-1, and phases 36-38 may legitimately add classes. Every
    class is also asserted individually, so the guard only has to catch the
    catastrophic case where introspection returns nothing at all.
    """
    assert len(_safemodel_classes()) >= 15
