...
- [x] 40-01-PLAN.md — Gate de alcance bloqueante (D-02 higyrus / D-12 `market_id`+`active`) antes de tocar ningún `pyproject.toml`; changelog de matriz desde cero, de-provisionalización de iol y market-data, bumps, `uv lock` único + `uv sync`, espejo de CI local, escaneo de credenciales y push de la branch nueva — wave 1
