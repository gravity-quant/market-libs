...
- **Disposición firmada por el operador (checkpoint Task 3, plan 39-07, 2026-08-30):** aprobado **NO-FIX / medido a medias**, con **destino nombrado `LIVE-POS-39`** — el requisito de una corrida en vivo con posiciones abiertas en la cuenta bbsa, único camino por el que el roster de la hoja `InstrumentPositionReport` puede medirse. Nombre elegido siguiendo la convención `LIVE-<PKG>-<NN>` ya usada por `LIVE-HIGY-33`, `LIVE-MATZ-33` y `LIVE-NOBJ-01`. El contenedor queda CONFIRMADO en esta fase; sólo la hoja queda diferida, con destino, según exige D-08.
...
- **Disposición firmada por el operador (checkpoint Task 3, plan 39-07, 2026-08-30):** aprobado **NO-FIX terminal**, sin ruteo a una fase posterior. El wire-superset es tolerado por diseño (`_SafeModel.from_api`) y la divergencia quedó reportada, no silenciada: no hay divergencia real que diferir, así que D-08 no exige destino nombrado. Si en el futuro la app necesita exponer `hasError`, se ensancha el roster en el ciclo de release que corresponda.
...
- **Diff:** backend posiblemente agregó campo nuevo; candidato a extender model
- **Disposición (Phase 39 / plan 39-07):** Clave del wire que `AccountReport` no declara, observada por primera vez en esta corrida (la captura viva de los endpoints Risk era imposible hasta que D-02 amplió el allowlist D-MATZ-33). Wire-superset tolerado por diseño y reportado, no silenciado. Ensanchar el roster es un cambio de superficie pública de un paquete publicado y pertenece al ciclo de release, no a un plan de verificación: elevado al operador en el checkpoint de la Task 3.
- **Disposición firmada por el operador (checkpoint Task 3, plan 39-07, 2026-08-30):** aprobado **NO-FIX terminal**, sin ruteo a una fase posterior. Mismo fundamento que F-24: el wire-superset es tolerado por diseño y la divergencia quedó reportada, no silenciada, así que no hay divergencia real que diferir y D-08 no exige destino nombrado.
