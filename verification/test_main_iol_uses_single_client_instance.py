"""REFAC-05 -- ``main_iol.py`` MUST construct at most one sync ``Client`` and one
async ``AsyncClient`` (the single-Client invariant, D-01/D-02).

Pre-migration every probe reached the module singleton independently via
``iol_client.client._get_default()`` / ``aio._get_default()``. The driver
migration (Phase 15) replaces those with exactly one ``Client()`` built in
``main()`` and one ``AsyncClient()`` built in ``_async_main()``, both threaded as
parameters into every ``probe_*``. This AST gate is the merge-blocking invariant:
it counts constructor ``Call`` sites for the two client classes across the whole
...
``mod.AsyncClient()`` (``ast.Attribute``). An accessor or builder method call is
not a constructor -- the one named by its ``.``-suffixed callee (an
``ast.Attribute`` whose ``attr`` is the accessor, not a class name) is therefore
excluded by construction because its attribute is not in the constructor-name
set.
"""
...
def test_main_iol_uses_single_client_instance() -> None:
    ...
    zero classes (all via ``_get_default()``) and so FAILS RED. The migrated
    driver constructs exactly two (one sync ``Client`` + one async
    ``AsyncClient``) and PASSES GREEN.
    """
