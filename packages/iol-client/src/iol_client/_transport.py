...
Phase 8 iol. RELY-01..04 + D-01/D-04/D-07/D-08/D-19/D-31. Mechanical duplicate
of the ámbito canary (Plan 8-02) with package-specific logger name. iol-paquete
delta: `_LOGGER_NAME = "iol_client"` so structured log records land on the iol
package logger (which has its own `RedactingFilter` attached via `_logging.attach()`
covering Bearer + password + OAuth `refresh_token` in URL + JSON formats).

Invariants (identical to ámbito):
...
- Mutation gate (D-01 / RELY-03): ``request.extensions["idempotent"]`` set by the
  ...
  pass through with no retry loop.
- Retryable surfaces (D-07): HTTP status ``408/409/429/5xx`` + transport-level
  ...
  (``IOLAPIError``/``IOLAuthError``/``IOLRateLimitError``) NEVER reach this layer.
...
- 401 re-auth (D-02): NOT this layer. ``IOLAuthError`` NEVER in ``retry_on=`` —
...
flow through ``logging.getLogger("iol_client")`` which has the
...
class _RetryableStatus(Exception):
    ...
    NOT a subclass of ``IOLAPIError`` — keeps D-07 invariant (domain exceptions
    never in ``retry_on=``). Carries the ``response`` so the outer
