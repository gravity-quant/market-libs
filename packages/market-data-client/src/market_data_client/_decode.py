"""The canonical per-field decode walker for market-data-client (Phase 29, DEC-01).
...
Who imports it: :mod:`market_data_client.models` only. ``SafeModel.from_api``
...
class DecodeScope:
    ...
    def __call__(self, model: str, path: str, kind: str, declared: str, observed: str) -> None:
        ...
        if kind not in _INFO_KINDS and STRICT_DECODE.get():
            raise MarketDataDecodeError(path, declared, observed, model)
...
STRICT_DECODE: ContextVar[bool] = ContextVar("market_data_client_strict_decode", default=False)
DECODE_SCOPE: ContextVar[DecodeScope | None] = ContextVar(
    "market_data_client_decode_scope", default=None
)
