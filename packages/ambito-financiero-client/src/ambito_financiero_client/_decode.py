"""The canonical per-field decode walker for ambito-financiero-client (Phase 29, DEC-01).
...
Who imports it: **nothing — the copy is dormant here, by design.** This
paquete's entire public surface is one function returning a ``float``, so
there is no ``models`` module and nothing calls the walker. The copy exists so
that the intactness gate covers five copies with one canonical shape to
normalize to, and so that a future typed surface inherits a proven decoder
instead of bootstrapping one. Dormant is not untested: the suite beside this
module drives every branch through locally declared fixture dataclasses.
...
from ambito_financiero_client.exceptions import AmbitoFinancieroDecodeError
...
class DecodeScope:
    ...
    def __call__(self, model: str, path: str, kind: str, declared: str, observed: str) -> None:
        ...
        if strict:
            raise AmbitoFinancieroDecodeError(path, declared, observed, model)
...
STRICT_DECODE: ContextVar[bool] = ContextVar(
    "ambito_financiero_client_strict_decode", default=False
)
DECODE_SCOPE: ContextVar[DecodeScope | None] = ContextVar(
    "ambito_financiero_client_decode_scope", default=None
)
