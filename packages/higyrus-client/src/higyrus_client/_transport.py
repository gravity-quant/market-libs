...
Phase 8 higyrus. RELY-01..04 + D-01/D-04/D-07/D-08/D-19/D-31. Mechanical
duplicate of the iol Plan 3 transport (which itself mirrors the ámbito Plan 2
canary) with package-specific logger name. higyrus-paquete delta:
``_LOGGER_NAME = "higyrus_client"`` so structured log records land on the
higyrus package logger (which has its own ``RedactingFilter`` attached via
``_logging.attach()`` covering Bearer + JSON ``{"password":"..."}`` (login body
per ``_core.py:189-197``) + JSON ``{"token":"..."}`` (login response) + URL
query ``cuit=...`` PII redaction).

Invariants (identical to ámbito/iol):
...
- 401 re-auth (D-02): NOT this layer. ``HigyrusAuthError`` NEVER in ``retry_on=``
  — caught at the shell ``_request()`` which clears ``state.token`` + re-runs
  ``_ensure_token()`` + retries once. See ``client.py`` / ``aio.py``.
...
All log records flow through ``logging.getLogger("higyrus_client")`` which has
...
class _RetryableStatus(Exception):
    ...
    NOT a subclass of ``HigyrusAPIError`` — keeps D-07 invariant (domain
